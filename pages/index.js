...
export default function Home() {
  ...
  // generate the default gallery (16 cards spanning every category) on mount
  useEffect(() => {
    const types = ["gradient", "pattern", "mesh", "animated"];
    const generated = [];
    for (let i = 0; i < 16; i++) generated.push(makeCard(types[i % types.length]));
      ...
      <section className="max-w-3xl mx-auto px-5 sm:px-8 py-10 border-t border-white/10">
        ...
        <div className="space-y-3 text-sm leading-relaxed text-white/65">
          <p>
            ...
            and animated mesh you see here is generated by code the moment the page loads —
            nothing is pulled from a database or an image library, and nothing you create is
